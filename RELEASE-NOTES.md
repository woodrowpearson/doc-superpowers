...
Two hook fixes that together close the "git pull / branch-switch freezes the terminal" class of bug on large doc indexes: (1) generated hooks now resolve the latest installed plugin-cache version at runtime instead of pinning the install-time version, and (2) the `post-merge` / `post-checkout` hooks now scope `check-freshness` to the files the merge/checkout actually changed instead of walking the entire index synchronously.
