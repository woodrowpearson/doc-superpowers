...
|------|---------|---------------|
| `SKILL.md` | Core skill logic: discovery phase, 11 action handlers (init, audit, review-pr, update, diagram, sync, hooks, release, spec-generate, spec-inject, spec-verify), agent prompt templates, verification gates, error handling | Adding/changing actions, modifying agent behavior, updating discovery logic |
| `scripts/doc-tools.sh` | Bundled freshness tooling with 4 subcommands: `build-index`, `check-freshness`, `update-index`, `status`. Content hashing for docs, commit SHA for code | Changing staleness detection, index schema, adding subcommands |
...
|-------------|----------|
| How actions are routed | `SKILL.md` Section 1 "Action Routing" |
| Discovery phase logic | `SKILL.md` Section 0 "Discovery Phase" |
| Agent prompt template | `references/agent-prompt-template.md` |
| Verification gate | `SKILL.md` Section 2 "Verification" |
...
| Error handling | `SKILL.md` Section 3 "Error Handling" |
| Common mistakes | `SKILL.md` Section 4 "Common Mistakes" |
...
| Spec lifecycle routing | `SKILL.md` Section 1 "Spec Lifecycle Routing" + `spec-generate`, `spec-inject`, `spec-verify` subsections |
| Spec lifecycle integration guide | `references/spec-lifecycle-protocol.md` — wrapper author contracts, integration patterns |
| Spec status transitions | `SKILL.md` `spec-inject` and `spec-verify` subsections (Draft -> In Review -> Implemented) |
| Spec supersession logic | `SKILL.md` `spec-generate` subsection — replaces/superseded_by handling |
| Release action routing | `SKILL.md` Section 1 "`release`" subsection (lines 409-427) — commit range, semver bump, drafting agent, RELEASE-NOTES.md prepend |
| RELEASE-NOTES.md auditing | `SKILL.md` audit step 8 — parse latest version date, detect unreleased commits, emit P2 Incomplete |
| Release common mistakes | `SKILL.md` Section 4 "Common Mistakes" — cutting a release without `/doc-superpowers release`, not syncing CLAUDE.md/README.md |
| Archive conventions | `SKILL.md` Section 1 `init` and `update` — `docs/archive/{type}/` |
...
User invokes /doc-superpowers init
  ...
  → Section 1: Action Router → init
    → Dispatch 3 Explore agents (structure, tech, conventions)
