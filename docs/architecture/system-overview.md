<!-- Generated by doc-superpowers | 2026-04-05 | commit: 38cd9df -->
...
C4Context
  ...
  System_Ext(ci, "GitHub Actions", "CI/CD platform — 3 shell-based workflows (PR freshness, scheduled audit, index auto-update) + 4 Claude-powered AI workflows (audit+update, PR review, release notes, spec verification)")
...
| CI/CD Templates | GitHub Actions YAML | 7 workflow templates: 3 shell-based (PR freshness check, weekly scheduled audit, doc-index auto-update) + 4 Claude-powered AI workflows (feature branch audit+update, PR doc review with @claude interactive, release notes drafting, spec compliance verification). Shell templates reference locally-vendored `.github/scripts/doc-tools.sh` (copied by installer). All actions are SHA-pinned for supply-chain security. AI workflows include concurrency groups and author association gates. Claude-powered templates require `ANTHROPIC_API_KEY` as a GitHub Actions secret |
