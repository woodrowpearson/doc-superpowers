...
```bash
# Plugin install — resolve the newest cached version with a shell glob
# (do NOT use `fd`: ~/.claude/.gitignore ignores plugins/, so fd returns nothing)
DOC_TOOLS="$(printf '%s\n' ~/.claude/plugins/cache/doc-superpowers/doc-superpowers/*/scripts/doc-tools.sh | sort -V | tail -1)"

# Clone-and-symlink or direct copy (Options 1 and 2 above)
DOC_TOOLS=~/.claude/skills/doc-superpowers/scripts/doc-tools.sh
```
