...
8. If `scripts/hooks/install.sh` exists, run `install.sh status` and append a one-line summary: `Hooks: N/5 git, N/3 claude, N/8 ci`
...
| CI/CD | `--ci` | 8 workflows: 3 shell-based (PR freshness check, weekly audit, doc-index auto-update) + 5 Claude-powered (feature branch audit+update, PR doc review + @claude interactive, release notes drafting, spec compliance verification, PR full-cycle review+update+diagram+sync) |
...
The CI tier installs 8 GitHub Actions workflow templates into `.github/workflows/` and vendors `doc-tools.sh` into `.github/scripts/`. The first 3 are shell-based and reference the vendored script locally (no remote fetching). The remaining 5 are Claude-powered and require an `ANTHROPIC_API_KEY` GitHub Actions secret. All actions are SHA-pinned for supply-chain security. AI workflows include concurrency groups and `doc-review-pr.yml` gates `issue_comment` triggers with author association checks.
