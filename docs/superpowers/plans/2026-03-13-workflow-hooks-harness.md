...
> **Status:** Implemented with extensions. Implementation exceeded this plan's scope -- 4 additional Claude-powered CI templates were added (commit 696ba07), hook commands were updated with git-root resolution (commit 39fb590), and CI security was hardened with SHA-pinned actions, locally-vendored doc-tools.sh, author association gates, and concurrency groups (commit 4727d2f), a fifth Claude-powered CI workflow `doc-pr-full-cycle.yml` was added (commit 05d981b), and a custom merge driver for `docs/.doc-index.json` was integrated into the git tier install (commit d0fa416). v2.10.0 added the per-PR release-notes fragment producer (`doc-pr-release.yml` + 3 colocated helpers). v2.12.0 (in flight on `feat/granular-install`) adds granular CI install via `--workflows=<csv|all|none>`, `--helpers=<bool>`, `--force`, `--transient`, and persistent install-state in `.claude/doc-superpowers/installed.json` managed by a new `scripts/hooks/state.sh` module.
