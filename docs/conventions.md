...
| Tier | Flag | Scripts |
|------|------|---------|
| Git | `--git` | pre-commit, post-merge, post-checkout, prepare-commit-msg |
| Claude Code | `--claude` | pre-commit-gate.sh, session-summary.sh |
| CI/CD | `--ci` | doc-freshness-pr.yml, doc-freshness-schedule.yml, doc-index-update.yml |
...
| `In Review` | Implementation started, spec under active verification | `spec-inject --phase=plan` |
